...
- Commands are examples. Replace uppercase placeholders such as `FILE_ID`, `PERMISSION_ID`, `COMMENT_ID`, `REPLY_ID`, `DRIVE_ID`, or `REVISION_ID` with real values from earlier command output.
...
- `REFERENCE_NAME` is a Drive folder reference configured in the proxy UI. File IDs come from Drive search, create, or read responses.
